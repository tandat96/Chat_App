...
class RouteHandler{
	...
	async loginRouteHandler(request, response){
		...
		if(data.username === '' || data.username === null) {
			...
			try {
				const result = await queryHandler.getUserByUsername(data.username);//note
				if(result ===  null || result === undefined) {
					...
					if( passwordHash.compareHash(data.password, result.password)) {
						await queryHandler.makeUserOnline(result._id);//note
						response.status(CONSTANTS.SERVER_OK_HTTP_CODE).json({
							error : false,
							userId : result._id,
							message : CONSTANTS.USER_LOGIN_OK //note
						});
					} else {
						response.status(CONSTANTS.SERVER_NOT_FOUND_HTTP_CODE).json({
							error : true,
